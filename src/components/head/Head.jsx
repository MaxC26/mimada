...
export const Head = ({ content }) => {
  ...
  const formatTitle = (text) => {
    if (!text) return { firstPart: 'Realza tu', secondPart: 'Belleza Natural' }
    const words = text.split(' ')
    const midIndex = Math.ceil(words.length / 2)
    return {
      firstPart: words.slice(0, midIndex).join(' '),
      secondPart: words.slice(midIndex).join(' ')
    }
  }

  const { firstPart, secondPart } = formatTitle(textHead)
        ...
        <div className='flex flex-col lg:flex-row items-center justify-between gap-12 lg:gap-8 mt-10'>
          ...
          <div className='w-full lg:w-1/2 text-center lg:text-left z-10'>
            ...
            <p className='text-gray-600 text-lg md:text-xl leading-relaxed max-w-xl mx-auto lg:mx-0 mb-10'>
              {textHead01 ||
                'La belleza es autenticidad y confianza. Cuando te cuidas y te sientes bien, brillas con luz propia. Realza tu esencia y refleja tu mejor versión.'}
            </p>
