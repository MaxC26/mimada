...
import { apiEliminarInstructor } from '../../services/instructor'
...
const InstructorTable = ({ reloadTrigger, onEditInstructor }) => {
  ...
  const cargarInstructores = async () => {
    ...
    try {
      ...
      // Soporta respuesta directa en array o envuelta en propiedad data/instructores/usuarios
      const lista = Array.isArray(resp) ? resp : resp?.data || []
      setInstructores(lista)
    } catch (error) {
      console.error('Error al cargar instructores:', error)
      toast.error(
        error?.response?.data?.message || 'Error al obtener la lista de instructores'
      )
      setInstructores([])
    } finally {
      setLoading(false)
  ...
  const eliminarInstructor = async (id) => {
    ...
    try {
      ...
      setInstructores((prev) => prev.filter((inst) => inst.instructorId !== id))
            ...
            <table className='w-full text-left border-collapse'>
              ...
              <tbody className='divide-y divide-gray-50 text-sm'>
                {instructoresFiltrados.map((inst) => {
                  ...
                  const titulo = inst.titulo || '—'
                  const experiencia = inst.experiencia ?? null
                  ...
                  return (
                    <tr key={instId} className='hover:bg-[#faf7f5]/40 transition-colors'>
                      ...
                      <td className='px-6 py-3.5'>
                        {experiencia !== null ? (
                          <span className='inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-[#c2a381]/10 text-[#a58b6c] text-xs font-bold'>
