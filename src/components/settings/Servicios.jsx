...
const ServicioBlock = ({ index, name, imageName, charCount, maxChars, setFieldValue, nameFile, setNameFile, setFile, onChange }) => (
  <div className='bg-gray-50 rounded-xl p-5 border border-gray-100 space-y-4'>
    <p className='text-xs font-black text-gray-500 uppercase tracking-widest'>Servicio {index}</p>

    {/* Título */}
    <div>
      <div className='flex items-center justify-between mb-1.5'>
        <label className='text-xs font-bold text-gray-500 uppercase tracking-widest'>Nombre del Servicio</label>
      ...
      <Field
        ...
        maxLength={maxChars}
        onChange={onChange}
        className={`w-full border rounded-xl px-4 py-3 text-sm text-gray-800 placeholder-gray-400 outline-none transition-all focus:border-[#e43c8a] focus:ring-2 focus:ring-[#fce7f3] ${charCount > maxChars ? 'border-red-400' : 'border-gray-200'}`}
      />
    </div>

    {/* Imagen */}
    <div>
      <label className='text-xs font-bold text-gray-500 uppercase tracking-widest block mb-2'>Imagen</label>
      <ImageUploader
...
const Servicios = ({ contenido, setIsLoading }) => {
  ...
  const [nf01, setNf01] = useState(''); const [nf02, setNf02] = useState('')
  const [nf03, setNf03] = useState(''); const [nf04, setNf04] = useState('')
  const [f01, setF01] = useState(null); const [f02, setF02] = useState(null)
  const [f03, setF03] = useState(null); const [f04, setF04] = useState(null)

  const [t01, setT01] = useState(texto01?.valor ?? ''); const [t02, setT02] = useState(texto02?.valor ?? '')
  const [t03, setT03] = useState(texto03?.valor ?? ''); const [t04, setT04] = useState(texto04?.valor ?? '')

  const [c01, setC01] = useState(t01.length); const [c02, setC02] = useState(t02.length)
  const [c03, setC03] = useState(t03.length); const [c04, setC04] = useState(t04.length)

  const MAX = 45

  const handlePreview = () => {
    const content = {
      textServicio01: t01, textServicio02: t02, textServicio03: t03, textServicio04: t04,
      imgServicio01: f01 ?? imgP01, imgServicio02: f02 ?? imgP02,
      imgServicio03: f03 ?? imgP03, imgServicio04: f04 ?? imgP04,
    }
          ...
          <Form className='space-y-5'>
            ...
            <div className='bg-white rounded-2xl p-6 border border-gray-100 shadow-sm'>
              ...
              <div className='grid grid-cols-1 md:grid-cols-2 gap-5'>
                <ServicioBlock
                  index={1} name='textServicio01' imageName='file01'
                  charCount={c01} maxChars={MAX}
                  setFieldValue={setFieldValue}
                  nameFile={nf01} setNameFile={setNf01}
                  setFile={setF01}
                  onChange={mkChange(setT01, setC01, setFieldValue, 'textServicio01')}
                />
                <ServicioBlock
                  index={2} name='textServicio02' imageName='file02'
                  charCount={c02} maxChars={MAX}
                  setFieldValue={setFieldValue}
                  nameFile={nf02} setNameFile={setNf02}
                  setFile={setF02}
                  onChange={mkChange(setT02, setC02, setFieldValue, 'textServicio02')}
                />
                <ServicioBlock
                  index={3} name='textServicio03' imageName='file03'
                  charCount={c03} maxChars={MAX}
                  setFieldValue={setFieldValue}
                  nameFile={nf03} setNameFile={setNf03}
                  setFile={setF03}
                  onChange={mkChange(setT03, setC03, setFieldValue, 'textServicio03')}
                />
                <ServicioBlock
                  index={4} name='textServicio04' imageName='file04'
                  charCount={c04} maxChars={MAX}
                  setFieldValue={setFieldValue}
                  nameFile={nf04} setNameFile={setNf04}
                  setFile={setF04}
