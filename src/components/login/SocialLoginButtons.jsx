...
/**
 * Botones reutilizables de inicio de sesión social (Google + Facebook).
 *
 * Props:
 *  - onSuccess : () => void       — se llama tras un login social exitoso
 *  - onError   : (msg) => void    — se llama si falla el login social
...
const SocialLoginButtons = ({ onSuccess, onError, className = '' }) => {
  ...
  return (
    <div className={`grid grid-cols-2 gap-3 ${className}`}>
      ...
      <button
        type='button'
        onClick={() => handleFacebookLogin({ onSuccess, onError })}
        className='flex items-center justify-center gap-2 border border-gray-200 rounded-xl py-2.5 text-sm font-semibold text-gray-700 hover:bg-gray-50 transition-colors'
      >
        {/* Facebook SVG Icon */}
